(function() {
  ...
  async function renderWalletCards() {
    ...
    try {
      ...
      const WALLET_META = {
        'shaun_poly':   { label: 'Shaun', platform: 'polymarket', initial: 300 },
        'shaun_kalshi': { label: 'Shaun', platform: 'kalshi', initial: 0 },
        'don_poly':     { label: 'Don S', platform: 'polymarket', initial: 0 },
        'don_kalshi':   { label: 'Don S', platform: 'kalshi', initial: 0 },
      };
      ...
      walletIds.forEach(wid => {
        ...
        // Per-wallet bankroll = initial deposit + sum(settled pnl)
        const walletBankroll = (meta.initial || 0) + totalPnl;
  ...
  async function renderAudit() {
    ...
    try {
      ...
      // ── Bankroll (initial deposit + realized PnL)
      const INITIAL_DEPOSIT = 300.0;
      const realized = settledAll.reduce((s,t)=>s+(t.pnl||0),0);
      const bankroll = INITIAL_DEPOSIT + realized;
      ...
      html += '<span>Initial deposit: $'+INITIAL_DEPOSIT.toFixed(0)+'</span>';
